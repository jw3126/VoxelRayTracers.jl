...
struct WallHit{N,T,V<:NTuple{N,Any}}
    position::NTuple{N,T}
    index::NTuple{N,Int}
    hittimes::NTuple{N,T}
    time::T
...
function next_wallhit(o::WallHit)
    ...
    position_new = o.velocity .* (time_new - o.time) .+ o.position
    s = (-1)^signbit(o.velocity[dim])
...
function _makeNTuple(::Type{NTuple{1,T}}, xs) where {T}
    x1, = xs
    (T(x1),)
end
function _makeNTuple(::Type{NTuple{2,T}}, xs) where {T}
    x1,x2 = xs
    (T(x1),T(x2))
end
function _makeNTuple(::Type{NTuple{3,T}}, xs) where {T}
    x1,x2,x3 = xs
    (T(x1),T(x2), T(x3))
end
function _makeNTuple(::Type{NTuple{4,T}}, xs) where {T}
    x1,x2,x3,x4 = xs
    (T(x1),T(x2), T(x3), T(x4))
end
function _makeNTuple(::Type{NTuple{4,T}}, xs) where {T}
...
function Base.iterate(o::EachTraversal)
    ...
    position = o.position .+ entry_time .* o.velocity
    ...
    item::eltype(o) = (voxelindex=CartesianIndex(index),
        entry_time=entry_time, exit_time=exit_state.time)
    item, exit_state
end
...
function _first_wallhit(position, velocity, edges, index, time)
    ...
    hittimes = interval_hit_times.(position, velocity, limits) .+ time
...
function Base.iterate(o::EachTraversal, entry_state::WallHit) where {N,T}
